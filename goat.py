...
# Specify IP of OOB/Remote Management interface
...
# Get available version from vendor (e.g. BIOS)
parser.add_argument("--avail", required=False)
# Check current version against available version from vendor (e.g. BIOS)
parser.add_argument("--check", required=False)
# Specify model (can be used with --avail)
parser.add_argument("--model", required=False)
# Specify port to run service on
...
# Mask serial and hostname output output
parser.add_argument("--mask", action='store_true')
parser.add_argument("--mesh", action='store_true')     # Use Meshcommander
# Display options information
parser.add_argument("--options", action='store_true')
# Automate via .goatpass
parser.add_argument("--allhosts", action='store_true')
# Start a SOL connection to host
...
def print_version(script_exe):
    ...
    version = list(filter(lambda x: re.search(r"^# Version", x), file_array))[
        0].split(":")[1]
...
def print_options(script_exe):
    file_array = file_to_array(script_exe)
    opts_array = list(
        filter(lambda x: re.search(r"add_argument", x), file_array))
    print("\nOptions:\n")
    ...
    if avail == "bios":
        ...
        for html_line in html_data:
            ...
            if re.search("BIOS Update", html_text):
                link_stub = BeautifulSoup(
                    html_text, features='lxml').a.get("href")
...
def get_amt_value(get_value, ip, username, password, driver, http_proto, search):
    ...
    if http_proto == "http":
        ...
        port_no = "16993"
    base_url = "%s://%s:%s@%s:%s" % (http_proto,
                                     username, password, ip, port_no)
        ...
        for html_line in html_data:
            ...
            if not re.search(r"hidden|onclick|colspan", html_text):
                ...
                plain_text = BeautifulSoup(
                    html_text, features='lxml').get_text()
...
def set_amt_value(set_value, ip, username, password, driver, http_proto, search):
    ...
    object.accept()
    driver.quit()
    string = "Sending %s to %s (Intel AMT has a 30s pause before operation is done)" % (
        set_value, ip)
    print(string)
    return
...
if option["type"]:
    ...
    for ip in ips:
        ...
        if oob_type == "amt":
            ...
            if option["check"]:
                model = get_amt_value(
                    "model", ip, username, password, driver, http_proto, search)
                current = get_amt_value(
                    check, ip, username, password, driver, http_proto, search)
            ...
            if option["avail"]:
                ...
                get_web_amt_value(avail, model, driver)
            if option["get"]:
                get_amt_value(get_value, ip, username, password,
                              driver, http_proto, search)
            if option["set"]:
                set_amt_value(set_value, ip, username, password,
                              driver, http_proto, search)
else:
